...
class Mainloop:
    ...
    def recognize(self, frame, count):
        ...
        for face_location, face_encoding in zip(face_locations, face_encodings):
            ...
            person_id = self.api_handler.lookup_face(
                face_encoding, self.known_face_encodings)
            # When we know the person
            if person_id:
                self.api_handler.add_facedetection(person_id, self.video_id, round(
                    count/self.stream_handler.FPS, 2), top, right, bottom, left)
            ...
            # Register the person if its unknown
                ...
                self.known_face_encodings.append(
                    {'encoding': face_encoding, 'person_id': person_id})
                self.api_handler.add_facedetection(person_id, self.video_id, round(
                    count/self.stream_handler.FPS, 2), top, right, bottom, left)
    ...
    def process_stream(self, end_time, video_title):
        self.log((self.stream_handler.FPS, self.stream_handler.WIDTH,
                 self.stream_handler.HEIGHT))
        video_path = config.VIDEO_PATH + video_title
        output_video = cv2.VideoWriter(video_path, cv2.VideoWriter_fourcc(
            *'XVID'), self.stream_handler.FPS, (self.stream_handler.WIDTH, self.stream_handler.HEIGHT))
    ...
    def main(self):
        ...
        while True:
            try:
                ...
                self.video_id = self.api_handler.add_video(
                    video_title.replace('.avi', '.mp4'))
